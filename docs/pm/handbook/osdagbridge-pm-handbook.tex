...
\vfill
{\large Version 1.2 \quad|\quad 1 August 2026}\\[4pt]
...
like an assembly line, and \emph{one} line carries a card all the way from ``nobody
has looked at this'' to ``it is running in production.'' There is no second
shipped-or-not column: the release stages are simply the tail of the same Status
axis.
...
\begin{tikzpicture}[font=\small,>={Latex[length=2mm]}]
  ...
  \node[st,fill=peGreenBg,right=5mm of v] (ld) {Live in Dev};
  \foreach \a/\c in {b/t,t/r,r/p,p/v,v/ld} \draw[->,brandblue] (\a) -- (\c);
  % bottom row: the release tail, wrapping from Live in Dev
  \node[st,fill=peGreenBg,below=9mm of b] (q) {QA / Verify};
  \node[st,right=5mm of q] (rt) {Ready for Test};
  \node[st,right=5mm of rt] (rp) {Ready for Prod};
  \node[st,fill=peGreenBg,right=5mm of rp] (ip) {In Production};
  \node[st,fill=peGreenBg,right=5mm of ip] (d) {Done};
  \foreach \a/\c in {q/rt,rt/rp,rp/ip,ip/d} \draw[->,brandblue] (\a) -- (\c);
  \draw[->,brandblue] (ld.south) |- (q.west);
  % waiting states, off to the side
  \node[st,fill=gotchaBg,right=16mm of ld] (bl) {Blocked};
  \node[st,fill=gotchaBg,below=4mm of bl] (oh) {On-Hold};
  \draw[->,gotchaRed,dashed] (p) -- (bl);
\end{tikzpicture}}
\end{center}

The first five columns are the everyday development flow. The moment code merges to
the default branch it becomes \textbf{Live in Dev} --- the same card keeps going,
now through the release stages: verified (\textbf{QA / Verify}), promoted to the
test channel (\textbf{Ready for Test}), cleared for release (\textbf{Ready for
Prod}), and finally shipped (\textbf{In Production}), before it lands in
\textbf{Done}. One card, one column, cradle to production.

\textbf{Blocked} and \textbf{On-Hold} are off to the side on purpose. They are not
stages you pass through, they are places a card waits: \emph{Blocked} when something
outside it is in the way (and it must name what it is waiting on), \emph{On-Hold}
when the team has deliberately paused it.
...
carry. We use thirteen. For each one below you get what it is, why it exists, its
type, and its values.
...
Type & single-select & What \emph{kind} of work it is: Epic, Feature, Story, Bug, Task, Chore, Spike, Docs, Test. Mirrors the \code{type:} labels so the board can \emph{group} by kind. \\
...
Team & single-select & Which team owns it (UI, Design Core, \ldots). Derived from the project's \code{teams:} block. \\
...
\begin{plainenglish}
\textbf{One axis, not two.} Earlier versions of this board split ``is it written?''
from ``is it shipped?'' into two separate columns --- a Status field and a Deploy
stage field. We folded them into one. Now a single \textbf{Status} carries a card
from Backlog all the way to In Production: once code merges it becomes \emph{Live in
Dev}, and the release stages (\emph{Ready for Test}, \emph{Ready for Prod}, \emph{In
Production}) are just the far end of the same line. The win is that there is only
ever one honest answer to ``where is this?'' --- you never have to reconcile two
columns that disagree. (If you are looking at an older board that still has a
separate Deploy stage field, the reconciler leaves it in place --- it never deletes
--- but nothing new is written to it.)
\end{plainenglish}
...
\chapter{Build the nine views}

A \textbf{view} is a saved arrangement: a layout plus a filter, so a specific
audience sees exactly the cards they care about. All nine are created by the
...
By Type & Board & Every open issue grouped by kind (epic, bug, feature, \ldots) & No open issues \\
...
\begin{gotcha}
\textbf{The grouped views need one manual toggle each.} The automation sets each
view's \emph{filter}, but GitHub exposes no way for a program to set a view's
\emph{grouping}. So after setup, open each of these, click \textbf{\ldots}
\code{->} \textbf{Group by}, and pick its field: \textbf{Workload} \code{->}
\textbf{Assignees} (or a custom Owner field --- see the interns chapter in Part~V);
\textbf{By Team} \code{->} \textbf{Team}; \textbf{By Type} \code{->} \textbf{Type};
and \textbf{Release pipeline} \code{->} \textbf{Status}. Same class of one-time
click as the five workflows in the next chapter.
\end{gotcha}
...
\begin{enumerate}[leftmargin=1.7em,itemsep=5pt]
  ...
  \item \textbf{Item closed becomes Status: Done.}\\
        ...
        Enable.
  \item \textbf{Pull request merged becomes Status: Live in Dev.}\\
        Workflows \code{->} \textbf{Pull request merged} \code{->} Edit. Set Status
        to Live in Dev. Enable.\\
        \emph{Why:} on the single-axis board a merged PR is not finished, it is
        \emph{shipped to the dev channel}. The release pipeline then carries it on
        through the tail (Ready for Test \code{->} In Production); closing the issue
        is what finally moves it to Done.
...
\chapter{How to use it: your first day on the board}

The board is built, the fields are in place, the views exist. Now: what do you
actually \emph{do} with it? This chapter is the hands-on part. If you have read
nothing else, read this. Everything after it goes deeper on the same actions.

Everything below happens in the browser, on the project board. You do not need the
command line, a token, or any of the configuration files to do daily work --- those
are for the person who \emph{sets up} the board, not the person who \emph{uses} it.

\section{Open the board and get your bearings}

Open the project. Along the top you will see tabs --- these are the \textbf{views}
from the last part. You will live in a few of them:

\begin{center}
\begin{tabular}{@{}p{3.4cm}p{9.8cm}@{}}
\toprule
\textbf{Tab} & \textbf{Open it when you want to\ldots} \\
\midrule
...
\begin{plainenglish}
A view never hides or changes an issue --- it is just a lens. The same card can show
up in Current Sprint, Workload, and By Team at once; they are three windows onto one
pile, not three piles. Switching tabs never moves anything.
\end{plainenglish}

\section{Find the work that is yours}

Open \textbf{Workload} and look for your name --- your issues are grouped under it.
Nothing there? Then you have not been given a sprint issue yet; ask at planning, or
pull one from \textbf{Current Sprint} that has no owner. If you do not have a name
column at all, you have not been added to the board yet --- see the interns chapter
in Part~V for the one-time invite.

\section{Pick up an issue and move it across}

This is the single most common thing you will do. An issue moves left-to-right along
\fieldname{Status} as you work it:

\begin{enumerate}[leftmargin=1.7em,itemsep=4pt]
  \item \textbf{Take it.} Open the card, set yourself as owner (the
        \fieldname{Assignees} field, or the custom Owner field --- see Part~V), and
        drag it from \textbf{Ready} to \textbf{In Progress}.
  \item \textbf{Work it.} Write the code, open a pull request. When the PR is up for
        review, move the card to \textbf{In Review}.
  \item \textbf{Merge it.} When the PR merges, the board moves the card to
        \textbf{Live in Dev} for you (that is one of the five automations). You do
        not drag it yourself.
  \item \textbf{Let it ride the tail.} From there the release pipeline carries it on
        through \textbf{QA / Verify}, \textbf{Ready for Test}, \textbf{Ready for
        Prod}, and \textbf{In Production} as it is promoted through the conda
        channels. Closing the issue is what finally sends it to \textbf{Done}.
  \item \textbf{Stuck?} If something outside your control is in the way, move the
        card to \textbf{Blocked} and write \emph{what} you are waiting on in a
        comment. If the team has deliberately parked it, use \textbf{On-Hold}.
\end{enumerate}

\begin{gotcha}
\textbf{Do not skip a card straight to Done because ``the code is written.''} On this
board Status is one honest line from idea to production. ``Written and merged'' is
\textbf{Live in Dev}, not Done. Done means it has been through the release tail and
closed. If you jam things to Done early, the Release pipeline view goes blind and
nobody can tell what has actually shipped.
\end{gotcha}

\section{File a new issue}

Found a bug, or thought of work that needs doing? Open an issue in the source
repository (not on the board --- the board pulls it in automatically). Give it a
clear title and a body that says, for a bug, \emph{what you did}, \emph{what you
expected}, and \emph{what happened instead}. Within seconds it appears on the board
in the \textbf{Triage} column, waiting to be classified. That is the automation
doing its job; you do not add it to the board by hand.

\section{Fill in the fields (classify a card)}

When you triage or pick up a card, set its fields. This is what turns a flat list
back into a managed project. The ones you touch most:

\begin{center}
\begin{tabular}{@{}p{2.6cm}p{11cm}@{}}
\toprule
\textbf{Field} & \textbf{What to set} \\
\midrule
\fieldname{Type} & What kind of work it is: Bug, Feature, Story, Task, \ldots \\
\fieldname{Severity} & For bugs only: S1 (wrong answer shown as right, or a crash) down to S4 (cosmetic). \\
\fieldname{Area} & Which subsystem(s) it touches --- pick more than one if it spans two. \\
\fieldname{Team} & Which team owns it. \\
\fieldname{Epic} & Which big outcome it belongs to. \\
\fieldname{Size} & Rough effort. Anything \textbf{XL} should be split, not started. \\
\fieldname{Priority} & How soon: P0 now, down to P3 backlog. \\
\bottomrule
\end{tabular}
\end{center}

\begin{plainenglish}
You do not have to fill in every field the moment an issue is born. Triage sets the
few that matter (Type, Severity, Area, an owner); the rest can wait until the issue
is actually pulled into a sprint. An empty \fieldname{Points} or \fieldname{Target
date} is not a mistake --- it just means ``not decided yet.''
\end{plainenglish}

\section{A worked first hour}

Say you are handed issue \#312, ``girder self-weight looks wrong in the report.''

\begin{enumerate}[leftmargin=1.7em,itemsep=3pt]
  \item It is sitting in \textbf{Triage}. You open it and classify: \fieldname{Type}
        = Bug, \fieldname{Severity} = S1 (a wrong structural number shown as if
        correct), \fieldname{Area} = \code{reports} and \code{design-core},
        \fieldname{Team} = Reports \& BOQ, \fieldname{Epic} = E1
        result-traceability. You set yourself as owner.
  \item At planning it is pulled into the sprint and you drag it to
        \textbf{In Progress}.
  \item You find the bad key binding, fix it, and open a PR. You move the card to
        \textbf{In Review}.
  \item The PR merges; the board moves \#312 to \textbf{Live in Dev} on its own.
  \item It rides the release tail to \textbf{In Production}. You close the issue and
        it lands in \textbf{Done} --- fixed, shipped, and traceable to the clause it
        implements.
\end{enumerate}

\begin{verifybox}
You have used the board correctly if, at any moment, someone can open it and answer
three questions without asking you: \emph{what are you working on} (your card in
In~Progress), \emph{is it stuck} (Blocked, with a named reason), and \emph{has it
shipped} (its place in the release tail). If all three are answerable from the board
alone, you are doing it right.
\end{verifybox}
...
\midrule
\endhead
\code{config/base.yml} & \textbf{Shared across every project}: the \code{type:} and \code{sev:} labels, all the board fields (Status, Sprint, Type, Priority, \ldots, Team), the sprint schedule, and the nine views. \\
\code{config/software/osdagbridge.yml} & \textbf{This project only}: its display name, its \code{area:} labels, its epics, its \code{teams:}, its \code{source\_repos}, and its conda channels. The board's Epic, Area, and Team options are \emph{derived} from here, so you never write them twice. \\
...
\textbf{Status columns (one axis):} Backlog \code{->} Triage \code{->} Ready
\code{->} In Progress \code{->} In Review \code{->} Live in Dev \code{->} QA /
Verify \code{->} Ready for Test \code{->} Ready for Prod \code{->} In Production
\code{->} Done. \emph{Blocked} and \emph{On-Hold} wait to the side; a blocked card
names its blocker. The release stages are the tail of Status --- there is no
separate deploy column.\\[4pt]
...
OsdagBridge Project Management Handbook, version 1.2. Built from the SOPs and board
