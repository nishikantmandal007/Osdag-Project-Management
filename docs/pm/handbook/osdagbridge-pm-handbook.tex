...
\vfill
{\large Version 1.0 \quad|\quad 1 August 2026}\\[4pt]
{\small Repository: \code{nishikantmandal007/osdagbridge-pm}}\\[2pt]
{\small Board: project \#3, ``OsdagBridge Delivery''}
\vspace*{1.5cm}
\end{titlepage}
...
on live next to it as \code{SOP-01} through \code{SOP-08} and
...
\begin{enumerate}[leftmargin=1.6em,itemsep=3pt]
  ...
  \item \textbf{The repository itself}, \code{osdagbridge-pm}. It is deliberately
...
\chapter{Add the eleven fields}

A \textbf{field} is a column: one piece of structured information every card can
carry. We use eleven. For each one below you get what it is, why it exists, its
type, and its values.
...
tool versions. The bootstrap script (\code{python -m pm.bootstrap\_board}) builds
all eleven from \code{config/project.yml} in one shot, and running it twice changes
nothing the second time. If you are doing it by hand, create the eight
single-select and number fields yourself and let the script add the three special
ones.
\end{notebox}
...
\midrule
\endhead
Status & single-select & Which column the card is in. Backlog, Triage, Ready, In Progress, In Review, QA / Verify, Blocked, Done. \\
...
\chapter{Build the six views}

A \textbf{view} is a saved arrangement: a layout plus a filter, so a specific
audience sees exactly the cards they care about. All six are created by the
...
\begin{enumerate}[leftmargin=1.7em,itemsep=5pt]
  \item \textbf{Auto-add items from the repository.}\\
        ...
        issue is added to \code{osdagbridge-pm} (filter \code{is:issue}), add it to
        the project. Enable.\\
...
OsdagBridge is open source, and the board is public. That raises three fair
questions. Here are straight answers.
...
\addlinespace
Seeder (\code{pm-seed}) & Migrated the 59 existing issues in, once & Necessary once, to avoid re-typing a backlog by hand. \\
\addlinespace
...
\begin{center}
\begin{longtable}{@{}p{4.2cm}p{9.0cm}@{}}
\toprule
\textbf{File} & \textbf{Controls} \\
\midrule
\endhead
\code{config/labels.yml} & Every label: the \code{type:}, \code{sev:}, \code{area:} families, and how old labels map to new ones. \\
\code{config/epics.yml} & The ten epics and E1's six sub-epics. \\
\code{config/project.yml} & The board: all eleven fields and six views. \\
\code{config/seed.yml} & The one-time import of existing issues. \\
\code{docs/pm/SOP-01..08} & The terse checklists this handbook expands on. \\
...
Progress \code{->} In Review \code{->} QA / Verify \code{->} Done. \emph{Blocked}
waits to the side and names its blocker.\\[4pt]
